    ...
    def update(
        ...
        for _ in range(cfg.ppo_epochs):
            for mb in iter_minibatches(
                ...
                log_ratio = torch.clamp(new_logp - mb.old_logprobs, -20, 20) # [B_mb, L-1]

                ratio = torch.exp(log_ratio)
                adv = torch.unsqueeze(adv, dim=-1) # [B_mb, 1]
                
                unclipped = ratio * adv # [B_mb, L-1]
                clipped = torch.clip(ratio, 1 - cfg.clip_eps, 1 + cfg.clip_eps) * adv # [B_mb, L-1]
                ...
                clipfrac = (torch.where(torch.logical_or(ratio < 1 - cfg.clip_eps, ratio > 1 + cfg.clip_eps), 1., 0.) 
                            * mask).sum() / mask.sum()
