...
def compute_per_token_logprobs(
    ...
    *,
    ...
    # autograd graph.
    whether_grad = torch.enable_grad() if enable_grad else torch.no_grad()
    with whether_grad:
        out = model(input_ids=input_ids, attention_mask=attention_mask, use_cache=False)
        naive_imple = torch.log_softmax(out.logits[:, :-1, :], dim=-1)
        result = torch.gather(naive_imple, -1, input_ids[:, 1:].unsqueeze(-1)).squeeze(-1)
    return result
...
def build_completion_mask(
    ...
    first_mask = pos >= prompt_input_len - 1
    not_pad = attention_mask[:, 1:].bool()
    comp_mask = torch.logical_and(first_mask, not_pad).float()
    return comp_mask
