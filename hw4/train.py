...
def compute_group_advantages(rewards: torch.Tensor, group_size: int, eps: float = 1e-6) -> torch.Tensor:
    ...
    if group_size <= 1:
        return rewards
    num_groups = int(rewards.numel() / group_size)
    shaped_rewards = rewards.reshape(num_groups, group_size)
    group_adv = (shaped_rewards - torch.mean(
        shaped_rewards, dim=1, keepdim=True)) / (torch.std(
            shaped_rewards, dim=1, keepdim=True) + eps)
    group_adv = group_adv.flatten()

    num_zako = rewards.numel() % group_size
    zako = rewards.flatten()[-num_zako: ]
    zako = (zako - zako.mean()) / (zako.std() + eps)
    return torch.cat([group_adv, zako])
